...
from envs.test_env.Test_env import TestEnv
from runners.separated.test_runner import TestRunner as Runner
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "test":
                env = TestEnv(all_args)
            else:
...
def parse_args(args, parser):
    parser.add_argument('--n_agents', type=int, default=5, help="the number of agent")
