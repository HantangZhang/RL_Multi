...
def discrete_parallel_act(decoder, obs_rep, obs, action, batch_size, n_agent, action_dim, tpdv,
    ...
    # 输入的action维度是 batch, n_agent, action，即6400条数据，每条数据有个5个agent做的5个动作
    # 如果batch是6400，nagent=5
    # action.squeeze(-1)把动作空间变为（6400，5）
    # one_hot后变为了(6400, 5, 5)， 对应的是每个智能体执行的动作，这个动作是个onthot，长度为5的一个向量表示一个动作[0,1,0,0,0]
    one_hot_action = F.one_hot(action.squeeze(-1), num_classes=action_dim)
    # 和onehot前两个维度一样，最后一维从action_dim变成了action_dim +1，加了一列
